#!/bin/bash
# start_agentbeats_green_ngrok.sh - Start Green Agent controller with ngrok tunnel
...
echo "=========================================="
echo "Starting Green Agent with ngrok Tunnel"
echo "=========================================="
...
nohup ngrok http $CTRL_PORT --log=stdout > logs/ngrok.log 2>&1 &
NGROK_PID=$!
...
ROLE=green AGENT_PORT=$AGENT_PORT HOST=$HOST PUBLIC_URL=$NGROK_URL \
    nohup bash -c "
        cd '$(pwd)'
        if [ -d 'venv' ]; then source venv/bin/activate; fi
        python backend/agentbeats_controller.py --port $CTRL_PORT --host 0.0.0.0 --public-url '$NGROK_URL'
    " > logs/green_controller_ngrok.log 2>&1 &
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo "❌ Controller failed to start. Check logs/green_controller_ngrok.log"
...
echo ""
echo "Logs:"
echo "  - Controller: tail -f logs/green_controller_ngrok.log"
