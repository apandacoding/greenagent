"""Green Agent using A2A framework - integrates with existing GreenAgent for AgentBeats compatibility."""
...
from chatbot.agent import GreenAgent, WhiteAgent
...
class GreenAgentExecutor(AgentExecutor):
    """A2A executor that wraps the existing GreenAgent for assessment."""
    
    def __init__(self):
        # Initialize the existing agents
        self.white_agent = WhiteAgent()
        self.green_agent = GreenAgent(white_agent=self.white_agent)
    
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        """Execute assessment task using the existing GreenAgent."""
        user_input = context.get_user_input()
        logger.info(f"[A2A Green] Received task: {user_input[:100]}...")
        
        try:
            # Process through existing GreenAgent
            result = await self.green_agent.process_message(user_input)
            
            # Extract the message from result
            message = result.get("message", "Assessment completed.")
            ...
            logger.error(f"[A2A Green] Error: {e}", exc_info=True)
            await event_queue.enqueue_event(
                new_agent_text_message(f"Error during assessment: {str(e)}")
            )
...
def prepare_green_agent_card(url: str = None):
    """Prepare Agent Card for Green Agent (Assessor)."""
    skill = AgentSkill(
        id="assessment",
    ...
    card = AgentCard(
        ...
        description="AI agent for travel planning using Green/White agent architecture - assesses white agent performance",
        url=url,
        version="1.0.0",
...
def start_green_agent(host: str = "0.0.0.0", port: int = 8001):
    """Start the green agent using A2A framework."""
    logger.info("Starting Green Agent with A2A framework...")
    ...
    card = prepare_green_agent_card(agent_url)
    ...
    agent_specific_card = prepare_green_agent_card(agent_specific_url)
    ...
    logger.info(f"Green Agent starting on {host}:{port}")
